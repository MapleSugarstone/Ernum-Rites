...
export type Action =
  ...
  | { type: 'PLAY_SUMMON'; handIndex: number; slot: number; targets?: TargetRef[] }
  | { type: 'CAST_SPELL'; handIndex: number; targets: TargetRef[] }
  | { type: 'PLAY_STAGE'; handIndex: number }
  | { type: 'ACTIVATE_POWER'; source: SourceRef; powerIndex: number; targets: TargetRef[] }
